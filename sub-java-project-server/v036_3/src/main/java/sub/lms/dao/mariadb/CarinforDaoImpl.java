...
public class CarinforDaoImpl implements CarinforDao{
  ...
  public CarinforDaoImpl(Connection con) {
    this.con = con;
  }
  ...
  public int insert(Carinfor carinfor) throws Exception {
    ...
    try(Statement stmt = con.createStatement()) {
      ...
      int result = stmt.executeUpdate( //
          "insert into carinfor(datas, departure,cdt, cnb, par)"
          +  " values( ' " + carinfor.getDatas().toString() + "', '" + carinfor.getDeparture().toString() +  "' , '"  + carinfor.getCarType() + "', '" + carinfor.getCarNumber() +"', '" +carinfor.getParking() + "'"
          + ")");
      return result;
    }
  }
