...
public class CustomerDaoImpl implements CustomerDao{
  ...
  public CustomerDaoImpl(String jdbcUrl, String username, String password) {
  ...
  @Override
  public List<Customer> findByKeyword(String keyword) throws Exception {
    try (Connection con = DriverManager.getConnection(jdbcUrl, username, password);
        Statement stmt = con.createStatement();
        ResultSet rs = stmt.executeQuery( //
            "select customer_id, car_tp, cnb, photo, " //
            + "discount, pay, gyeoljeyuhyeong, par, " //
            + "exit_car, accep from customer"
            + " where car_tp like '%" + keyword //
            + "%' or cnb like '%" + keyword //
            + "%' or discount like '%" + keyword //
            + "%' or pay like '%" + keyword //
            + "%' or gyeoljeyuhyeong like '%" + keyword //
            + "%' or par like '%" + keyword //
            + "%' or exit_car like '%" + keyword //
            + "%' or accep like '%" + keyword + "%'")) {
