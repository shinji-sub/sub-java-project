...
- src/main/java/sub/dao/PhotoFileDao.java 추가
- src/main/java/sub/domain/PhotoFile.java 추가 
- src/main/java/sub/dao/mariadb/PhotoFileDaoImpl.java 추가
- src/main/java/sub/dao/mariadb/PhotoBoardDaoImpl.java 변경
- src/main/java/sub/servlet/PhotoBoardDetailServlet.java 변경
...
### 훈련1: `고객 사진 게시판`에 사진 파일을 첨부하는 기능을 추가하라.

- sub.lms.domain.PhotoFile 추가
  - 사진 파일의 타입을 정의한다.
- sub.lms.dao.PhotoFileDao 인터페이스 추가
  - 사진 파일의 CRUD 관련 메서드 호출 규칙을 정의한다.
- sub.lms.dao.mariadb.PhotoFileDaoImpl 추가
...
- sub.lms.dao.mariadb.PhotoBoardDaoImpl 변경
  - insert() 메서드를 변경한다.
...
- sub.lms.ServerApp 변경
  - `PhotoBoardAddServlet` 객체에 CustomerDao와 PhotoFileDao 객체를 주입한다. 
...
- sub.lms.dao.PhotoFileDao 인터페이스 변경
  - 사진 파일 목록을 리턴하는 메서드를 추가한다.
  - findAll(int boardNo)
- sub.lms.dao.mariadb.PhotoFileDaoImpl 변경
  - PhotoFileDao 인터페이스에 추가된 메서드를 구현한다.
- sub.lms.servlet.PhotoBoardDetailServlet 변경
